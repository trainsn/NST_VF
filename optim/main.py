...
def optimize(args):
    ...
    for y in range(size):
        for x in range(size):
            ...
            if (rsq == 0):
                ...
                vectors[y, x, 1] = xx / rsq if yy!=0 else eps
    ...
    output = Variable(torch.randn(output_size, device="cuda") * 50 + 127, requires_grad=True)
    ...
    for e in tbar:
        ...
        vgg_input = utils.subtract_imagenet_mean_batch_device(output, args.cuda)
