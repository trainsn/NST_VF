...
def subtract_imagenet_mean_batch_device(batch, cuda):
    """Subtract ImageNet mean pixel-wise from a BGR image."""
    tensortype = type(batch.data)
    mean = tensortype(batch.data.size())
    mean[:, 0, :, :] = 103.939
...
def gray_bgr_batch(batch):
    """transform a BGR image batch to gray image batch L = R * 299/1000 + G * 587/1000 + B * 114/1000"""
    gray = 0.114 * batch[:, 0, :, :] + 0.587 * batch[:, 1, :, :] + 0.299 * batch[:, 2, :, :]
